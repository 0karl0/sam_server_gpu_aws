...
def save_masks(masks, image, base_name, masks_dir: str):
    """Save each mask individually without merging.

    The masks generated by SAM are resized to the original image size and
    written out as separate PNG files using a ``*_maskN.png`` naming scheme.
    ...
    for m in masks:
        ...
        if area < AREA_THRESH:
            continue
        out = seg * 255
        out_path = os.path.join(masks_dir, f"{base_name}_mask{mask_idx}.png")
        mask_idx += 1
    ...
    for f in new_files:
        ...
        try:
            ...
            if simple or not yolo_points:
                ...
                save_masks(masks, img, base, dirs["masks"])
            processed.add(base)
            count += 1
            gc.collect()
